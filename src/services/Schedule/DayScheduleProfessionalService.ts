...
class DayScheduleProfessionalService {
    async execute({ date, professionalId }: ScheduleRequest) {
        ...
        const startTime = new Date(`${format(date, 'yyyy-MM-dd')}T${workSchedule.startTime}`);
        const endTime = new Date(`${format(date, 'yyyy-MM-dd')}T${workSchedule.endTime}`);
        ...
        while (isBefore(currentTime, endTime) || isEqual(currentTime, endTime)) {
            ...
            const slotEndTime = addMinutes(currentTime, 60); // Duração de 60 minutos para o slot
            ...
            const nextEventTime = nextSchedule ? parse(nextSchedule.startTime, 'HH:mm', date) : null;
            const nextBookingTime = nextBooking ? parse(nextBooking.startTime, 'HH:mm', date) : null;
            
            // Verificar se há pelo menos 60 minutos livres antes do próximo evento
            const hasEnoughTimeBeforeNext = (nextEventTime && nextBookingTime)
                ? isBefore(addMinutes(currentTime, 45), nextEventTime) && isBefore(addMinutes(currentTime, 45), nextBookingTime)
                : nextEventTime
            ...
            const isBooked = bookings.some(booking =>
                (isAfter(currentTime, parse(booking.startTime, 'HH:mm', date)) && isBefore(currentTime, parse(booking.endTime, 'HH:mm', date))) || // currentTime dentro do intervalo de booking
                (isAfter(slotEndTime, parse(booking.startTime, 'HH:mm', date)) && isBefore(slotEndTime, parse(booking.endTime, 'HH:mm', date))) || // slotEndTime dentro do intervalo de booking
                (isEqual(currentTime, parse(booking.startTime, 'HH:mm', date)) || isEqual(slotEndTime, parse(booking.endTime, 'HH:mm', date))) || // Slot começa ou termina exatamente no horário de booking
                (isAfter(currentTime, parse(booking.startTime, 'HH:mm', date)) && isBefore(slotEndTime, parse(booking.endTime, 'HH:mm', date))) // Slot cobre o intervalo de booking
            );
